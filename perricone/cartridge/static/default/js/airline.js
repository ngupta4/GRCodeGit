                ...
                jQuery.getJSON(
                    ...
                    function(price) {
                        if (price.success) {
                            ...
                            if(jQuery("#daySupply option:selected").text().indexOf("90") != -1){
                            	var permonth=" <span class=\"per-month\">PER MONTH</span>";
                            	jQuery(app.proactiv.resources.get('total-price-selector')).html(price.update.totalPrice.stringValue+ permonth);                        		                          
                            }
                            else{
                            	jQuery(app.proactiv.resources.get('total-price-selector')).html(price.update.totalPrice.stringValue);
                            }
            ...
            jQuery('.popupOverlay').click(function () {
                ...
                app.createDialog({
                    ...
                    options: {
                        ...
                        height:500,
                        title: '',
                        dialogClass: 'popupOverlay-outer',
                        resizable: false
                    }
                });
    ...
    TermsConditionPopup: {
        ...
        init: function () { 
            jQuery('.terms-condition-overlay').click(function () {
                ...
                jQuery("#moreInfo").css('margin','14px 0 20px');
                var objtitle = jQuery("#moreInfo").siblings('.ui-dialog-titlebar');
                jQuery('#moreInfo').load(urlvedio);
                jQuery('#moreInfo').dialog('open');
                    ...
                    function (supply) {
                        if (supply.success) {
                    		...
                    		$(".upsellrecommendation .upsellInline li").removeClass('active');
                           	...
                           	var addPerMonth=(upsellClass=="90-Day")?" PER MONTH":"";
...
function updateProductInfo(supply){
	if(checkSupply(supply)){
		var permonth=" <span class=\"per-month\">PER MONTH</span>";
		jQuery(app.proactiv.resources.get('total-price-selector')).html(supply.update.TotalPrice.stringValue + permonth);
				
	} else {jQuery(app.proactiv.resources.get('total-price-selector')).text(supply.update.TotalPrice.stringValue);}
	jQuery(app.proactiv.resources.get('subtotal-selector')).text(supply.update.SubTotal.stringValue);
...
function checkSupply(supply){
	var pid=supply.update.CoreId.stringValue;
	for (i = 0; i < supply.update.daySupply.length; i++) {
		if((supply.update.daySupply[i].value == pid) && (supply.update.daySupply[i].label.indexOf('90') != -1) )
        {
			return true; break;
        }
    }
...
jQuery(document).ready(function() {
	...
	equalHeight($(".rightContent .content"));
	...
	$(".kitType > .selectKit > ul.sasKits > li").click( function(){
		...
		$("a#step4").click(function() {
			...
			$(".step2 a").click(function() {
				   scrollToAnchor('Step2');
				   s.linkTrackVars='prop56,eVar56';
				   s.prop56=campaignCode+":sas:SelectFreeGift";
				   s.eVar56=s.prop56;
			...
			$(".step3 a").click(function() {
				   scrollToAnchor('Step3');
				   s.linkTrackVars='prop56,eVar56';
				   s.prop56=campaignCode+":sas:SelectSupply";
				   s.eVar56=s.prop56;
				   s.tl(this,'o',"Select a Supply");
				   //$('.freeGiftCont .pageTitle').css('margin-top','0px')
			});
			$(".step4 a").click(function() {
				   scrollToAnchor('Step4');
				   s.linkTrackVars='prop56,eVar56';
				   s.prop56=campaignCode+":sas:Checkout";
				   s.eVar56=s.prop56;
			...
			$('.updatecartbtn').click(function() {				
			    ...
			    $(".freeGiftCont .orderSummaryBottom .kitTab").html($(".freeGiftID:visible .freeGiftSelector .sasKits .active .pdata .pTab").html());
			    $(".upsell-id:visible .upsellInline .ordersummarycontainer .kitTab").html($(".freeGiftID:visible .freeGiftSelector .sasKits .active .pdata .pTab").html());
			    
			    var noOfSupplyDays = $(this).attr("class").split(" ")[2];
			    if(noOfSupplyDays == '30-Day' || noOfSupplyDays == '90-Day'){
			    	$(".upsellInline .ordersummarycontainer .kitName .no-of-supply-days").html(noOfSupplyDays);
			    }else{
			    	$(".upsellInline .ordersummarycontainer .kitName .no-of-supply-days").html('30-Day');
			    }
...
/* upsellcode end here */
$(function(){
	$('.90-Day').bind('click',function(){  /*HACK FOR PER MONTH.  THERE'S ANOTHER ONE SOMEWHERE ABOVE*/
		$(".bottomPanel .prodSummary .kitPrice").append(" <span class=\"per-month\">PER MONTH</span>");
	 })
	});