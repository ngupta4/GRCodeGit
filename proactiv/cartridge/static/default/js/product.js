	...
	if (app) {
		...
		app.Product = function(response) {
			...
			var getOptionsDiv	= function(thisProduct) {
				...
				if (model.isOption) {
					...
					pdpOpt.change(function(e){
						...
						thisProduct.showUpdatedPrice(computePrice(thisProduct), model.pricing.standard, model.pricing.regular, model.pricing.member);
					});
			...
			var getAddToCartBtn = function(thisProduct) {
				var addToCartBtn = jQuery(thisProduct.containerId+" #pdpATCDivpdpMain"+" .addtocartbutton").click(function(e) {
					...
					//setting value for upsell, if value is AddUpsell then upsell model will be shown. for variant products upsell will be added to
					//anchor link but for non variants upsell will be added to add to cart button.
					if( model.master ||  model.variant){
						thisProduct.selectedOptions.upsell=$(this).parents(".productdetail").find(".swatchesdisplay li.selected a").attr('upsell');
					}
					else{
						thisProduct.selectedOptions.upsell=$(this).attr('upsell');
					}
			...
			return {
				...
				enableA2CButton: function() {
					jQuery(this.containerId+" #pdpATCDivpdpMain"+" .addtocartbutton").removeAttr("disabled");
				},
				...
				disableA2CButton: function() {
					jQuery(this.containerId+" #pdpATCDivpdpMain"+" .addtocartbutton").attr("disabled", "true");
				},
				...
				* */
				...
				refreshView: function() {
					...
					if (!isLoadingVar && this.selectedVar != null) {
						...
						// update price
						this.showUpdatedPrice(computePrice(thisProduct), this.selectedVar.pricing.standard,this.selectedVar.pricing.regular, this.selectedVar.pricing.member);
				 ...
				 * @param member - member price
				 */
				showUpdatedPrice: function(sale, standard, regular, member) {
					var standardPrice 	= Number(standard || 0);					
					var salePrice 		= Number(sale || 0);
					var regularPrice 	= Number(regular || 0);
					var memberPrice 	= Number(member || 0);
					var priceHtml 		= "";
					var formattedPrices = {"salePrice": salePrice, "standardPrice": standardPrice, "regularPrice":regularPrice, "memberPrice":memberPrice};
					...
					app.ajax.getJson({
						...
						cache	: true,
						async	: false,
						data	: {"salePrice": salePrice, "standardPrice": standardPrice, "regularPrice":regularPrice, "memberPrice":memberPrice},
						callback: function(data){
					...
					jQuery(this.containerId).find(".productleft .salespricevalue").text(formattedPrices.salePrice);
					jQuery(this.containerId).find(".productleft .regularpricevalue").text(formattedPrices.regularPrice);
					jQuery(this.containerId).find(".productleft .memberpricevalue").text(formattedPrices.memberPrice );					
					var regularPrice = parseFloat(formattedPrices.regularPrice.substring(1, formattedPrices.regularPrice.length));
					var memberPrice = parseFloat(formattedPrices.memberPrice.substring(1, formattedPrices.memberPrice.length));
					var percentageSaved = ((regularPrice-memberPrice)/regularPrice)*100;
					jQuery(this.containerId).find(".productleft .save-xx-value").text(Math.round(percentageSaved));
				},
				...
				*/
				...
				show: function(options) {
						...
						jQuery(thisProduct.containerId + " .variationattributes .swatches").each(function(){
							...
							var varEventHandler = function(e){									
								...
								if (thisObj.parent().hasClass("unselectable")) {
									...
									if( thisObj.parent().attr('lang') == "addtokit"){
										thisObj.parents('.productdetail').find('.kitdisplay').css('display','Block');
										thisObj.parents('.productdetail').find('.kitpricevalue').text(thisProduct.selectedVar.pricing.kitPrice);
									}else{
										thisObj.parents('.productdetail').find('.kitdisplay').css('display','none');
									}
								}																
					...
					//alert("this.getPrice(): "+this.getPrice() +"this.isPromoPrice(): "+this.isPromoPrice() +"master: "+(model.master || model.variant || model.productSet || model.bundle)+" set: "+(!model.inStock && model.avStatus === app.constants.AVAIL_STATUS_NOT_AVAILABLE && !model.productSet));
