...
CheckoutUtils = {
	...
	*/
	orderReviewLoad : function() {
		...
		jQuery("button.continuecheckout").click(function() {
			...
			if(app.resources["IS_USER_AUTHENTICATED"] == "false"){
				...
				var targetPipline = "CORtnOrderSummary-Submit";
				//Display login
				app.displayLogin(targetPipline);
	...
	changePaymentMethod : function(paymentMethodID) {
		...
		var eCheckMethodId = app.resources['ECHECK_METHOD_ID'];
		if( jQuery("#PaymentMethod_" + paymentMethodID).length == 0 )
		{
			CheckoutUtils.cleanCreditCardFormErrorMessages();
			CheckoutUtils.cleaneCheckFormErrorMessages
			jQuery("#PaymentMethod_Custom").show();
			agreeDivEnableDisable(false);
		}
        
		else if(paymentMethodID=="CREDIT_CARD"){
//			CheckoutUtils.cleaneCheckFormErrorMessages();
			jQuery("#PaymentMethod_"+eCheckMethodId).find('input').attr('disabled',true);
//			jQuery('.echeck_bankName').find(':input').attr('disabled', true);  
			agreeDivEnableDisable(false);
		}
		
		/*enable eCheck form and remove error message of CC*/
		else if(paymentMethodID == eCheckMethodId){
//			CheckoutUtils.cleanCreditCardFormErrorMessages();
			jQuery("#PaymentMethod_"+eCheckMethodId).find('input').attr('disabled',false);
			jQuery("#PaymentMethod_CREDIT_CARD span.errormessage").css('display','none');
			jQuery("#PaymentMethod_CREDIT_CARD .ccTypeError").css('display','none');
			jQuery("#PaymentMethod_CREDIT_CARD .invalidCC").css('display','none');
			/*var selectedBankName = jQuery('input.bankAccountTypeId').val();
			if((selectedBankName == 'Corporate' || selectedBankName == 'Corp Savings' )){
				jQuery('.echeck_bankName').show().find(':input').removeAttr('disabled');	
			  }
			else{
				jQuery('.echeck_bankName').hide().find(':input').attr('disabled', true);
				jQuery("#bankAccountType_Checking").attr('checked', true);
				}*/
			agreeDivEnableDisable(true);
		}
	...
	*/
	...
	cleanCreditCardFormErrorMessages : function() {
	    ...
	    /* This method is for cleaning the error messages from the eCheck page  */
		cleaneCheckFormErrorMessages : function() {
			jQuery("div.eCheckPayment").find("span.errormessage").each( function(){
				var errorMsg = jQuery(this);
				errorMsg.hide();
			});
		}
...
function agreeDivEnableDisable(enable){
	
	if(enable){
		jQuery('#termsAndConditionBankAccount').show();
		jQuery('#termsAndConditions').removeAttr("disabled");
	}
	else{
		jQuery('#termsAndConditions').attr("disabled","disabled");			
...
/*disabled and enabled Bank Name according to Bank A/C Type for eCheck(CMM)*/
/*
jQuery(document).ready(function() { 
    jQuery('.eCheck_bankAccountType input').click( function() {
    var selectedBankName = jQuery(this).val();
    if(selectedBankName == 'Corporate' || selectedBankName == 'Corp Savings' ){
		  jQuery('.echeck_bankName').show().find(':input').removeAttr('disabled');  
	  }
    else if(selectedBankName != 'Corporate' || selectedBankName != 'Corp Savings' ){
		  jQuery('.echeck_bankName').hide().find(':input').attr('disabled', true);  
	  }
    });
    });*/