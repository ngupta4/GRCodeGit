	...
	return {
		...
		minicart: {
			...
			addFromCLP: function(productID, availability, showUpsell){
				if(app.minicart.showupselloff == "true"){
					showUpsell = "false";
				}
			...
			showUpsellDialog: function(productID, quantity){
				...
				jQuery("#upSellrecommendation").empty();
			  	app.createDialog({id: 'upSellrecommendation', options: {
							    	...
							    	height:555,
							    	width:817,
							    	//title:'Upsell Recommendation',.proactivedialog
							    	resizable: false,
							    	dialogClass:"proactivedialog"
								}});
					jQuery('#upSellrecommendation').dialog('open');
					jQuery('#upSellrecommendation').parent().find(".ui-dialog-titlebar-close").css("top","10px");
			...
			add: function(progressImageSrc, postdata, callback)
			{
				...
				if (null == readCookie('dontshowupsell') && (postdata.upsell=="true") && app.minicart.showupselloff != "true"){
