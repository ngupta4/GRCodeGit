...
F<H;F++){if(FSR.site[A[F]]){FSR[A[F]]=FSR.site[A[F]]}}break}}if(B){break}}})();FSR.AdminController=new FSR.Class({Implements:FSR.Options,options:{},initialize:function(A){this.setOptions(A);
...
A.run()}})},run:function(){var O=new FSR.Hash.Cookie("fsr.sp",{path:"/",domain:FSR.site.domain||false});
var Q=new FSR.Element("table");var A=new FSR.Element("tbody");Q.fsr$adopt(A);var B=new FSR.Element("tr");
...
A.fsr$adopt(B);var R=FSR.surveydefs;for(var N=0,K=R.length;N<K;N++){if(R[N].site&&R[N].site!=FSR.site.name){continue
}var S=R[N].criteria;if(FSR.$defined(S.sp)){var L;var T;this.row(A,R[N].name,R[N].section||T,L,O,S.sp,S.lf)
}var I=S.locales||[];for(var M=0,J=I.length;M<J;M++){var T;this.row(A,R[N].name,R[N].section||T,I[M].locale,O,I[M].sp,I[M].lf)
}}var G=$fsr("controls");G.fsr$adopt(Q);var P=FSR.Cookie.read("fsr.ipo",{path:"/",domain:FSR.site.domain||false});
if(P==1){FSR.Array.each($$fsr("input"),function(U){if(U.type=="checkbox"&&U.name=="ipo"){U.checked=true
}})}var H=FSR.Cookie.read("fsr.pool",{path:"/",domain:FSR.site.domain||false});if(H==1){FSR.Array.each($$fsr("input"),function(U){if(U.type=="checkbox"&&U.name=="pool"){U.checked=true
}})}},unload:function(){fsr$dbug.log("Admin: unload")},row:function(F,A,H,M,K,D,Q){var J=new FSR.Element("tr");
...
}else{A.erase(B.name)}}else{if(B.type=="checkbox"){if(B.name=="ipo"){if(B.checked){document.cookie="fsr.ipo="+B.value+";path=/"+((FSR.site.domain)?";domain="+FSR.site.domain+";":";")
}else{FSR.Cookie.dispose("fsr.ipo",{path:"/",domain:FSR.site.domain||false})}}else{if(B.name=="pool"){if(B.checked){document.cookie="fsr.pool="+B.value+";path=/"+((FSR.site.domain)?";domain="+FSR.site.domain+";":";")
}else{FSR.Cookie.dispose("fsr.pool",{path:"/",domain:FSR.site.domain||false})}}}}}})},clear:function(){var A=new FSR.Hash.Cookie("fsr.sp",{path:"/",domain:FSR.site.domain||false});
A.empty();FSR.Cookie.dispose("fsr.pool",{path:"/",domain:FSR.site.domain||false});FSR.Cookie.dispose("fsr.ipo",{path:"/",domain:FSR.site.domain||false});
FSR.Array.each($$fsr("input"),function(B){if(B.type=="text"){B.value=""}else{if(B.type=="checkbox"){B.checked=false
}}})}});new FSR.AdminController({});window.fsr$addEvent("domready",function(){(function(){FSR.controller.load()
