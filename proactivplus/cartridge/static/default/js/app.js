...
var app = (function(jQuery){
	...
	return {
		...
		displayLogin : function(targetPipeline) {
    		...
    		el.setAttribute('width', '308');
    		el.setAttribute('height', '520');
	    	...
	    	jQuery('#checkoutlogincontent').dialog({
	    	  ...
	    	  var objtitle = jQuery("#checkoutlogincontent").siblings('.ui-dialog-titlebar');
				objtitle.find('.ui-dialog-titlebar-close').addClass('quickViewDialog-close').removeClass('ui-dialog-titlebar-close').addClass('blk-button');
				jQuery("#checkoutlogincontent").parent().css("background","#fff");
		},
		
		/*
		 * From legacy Mojo behavior /js/proactiv/global/behavior/flashDetectBehavior.js
		 * 
		...
		minicart: {
			...
			showUpsellDialog: function(productID, quantity){
				...
				jQuery("#upSellrecommendation").empty();
			  	...
			  	var objtitle = jQuery("#upSellrecommendation").siblings('.ui-dialog-titlebar');
				objtitle.find('.ui-dialog-titlebar-close').addClass('quickViewDialog-close').removeClass('ui-dialog-titlebar-close').addClass('blk-button');
		...
		quickView: {
			...
			bindEvents: function(options) {
				...
				jQuery(options.buttonLinkSelector).click(function(e) {
					...
					jQuery(options.buttonSelector).click(function(e) {
					var producturl =$(this).attr("data-url-type");
			...
			show: function(options) {
				...
				jQuery("#QuickViewDialog").css( "min-width", "700px" );				
				jQuery('#QuickViewDialog').dialog('open');				
				jQuery("#QuickViewDialog").parent().css('top', parseInt(windowheigh)+'px');
			    app.getProduct({containerId: "QuickViewDialog", source: options.source, url: options.url, label: options.label});
			    
			    //call it when resize or change the window orientation
			    app.winResize.init();
			},
				     ...
				     var arrMatches = strUserAgent.match(/(iphone|ipod|ipad)/);
				        if (arrMatches){
							...
							if(jQuery("#QuickViewDialog").length>0){
								var dialogTopPos= jQuery(window).height()/2 - 265;
								jQuery("#QuickViewDialog").parent().css('top', parseInt(dialogTopPos)+'px');
							}
				         }
				});
			}
		...
		validator: function() {
			...
			jQuery.validator.addMethod("errormessage-lang", function(value, element) {
				var langregexp = /^[a-zA-Z0-9\+\.\-\ &amp;\/]+$/;
				if (this.optional(element) && value == ''){
					return true;
				}
				
				return this.optional(element) || value.match(langregexp);
			}, app.resources["INVALID_LangAddress"]);
			 ...
			 *     $("#billZip, #shipZip").keyup(function () {
                var zipregexp =/^[0-9\-]+/g;
                var value=$(this).val();
               	$(this).val(value.match(zipregexp));
            });
			 */
	/*		jQuery.validator.addMethod("zip", function(value, element) {
			...
			jQuery.validator.addMethod("zip", function(value, element) {
			 ...
			 jQuery.validator.addMethod("zip", function(value, element) {
					...
					return this.optional(element) || value.match(zipwregexp);
				}, app.resources["INVALID_ZIPenter"]);
...
jQuery(document).ready(function(){
	...
	jQuery(".accountnav").mouseenter(function(){		
		app.minicart.close();
	});
	
	jQuery(".checkoutprogressindicator .active a").click(function(event){
	...
	if($("#home-catalog").length>0){
		quickViewOptions.imageSelector = "div.product div.image";
	}
		app.quickView.bindEvents(quickViewOptions);
