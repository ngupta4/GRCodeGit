...
function handleRTSErrorCode(rtserrors){
	...
	}else if((errorCode=='8210'||errorCode=='8209') && !ziperror){
		ziperror = true;
		 jQuery("<span/>").attr("class", "errormessage rts").html( errorMsg).appendTo('.formfield.zip .zip .value');
...
function display100dollarPopup(){
...
function display100dollarTerms(){
	jQuery('#dialogbox-payLater100Dollar').dialog('close');
