    ...
    TermsConditionPopup: {
        ...
        init: function () {
            jQuery('.terms-condition-overlay').click(function () {
                ...
                var urlvedio = jQuery(this).attr('href');
                jQuery("#moreInfo").parent().css('background', '#fff').css('padding', '10px 22px 12px 35px').css('overflow', 'hidden').css('margin-top', '35px');;
...
SCCOMMON.init = function () {
    $.each(SCCOMMON.ui, function (n, v) {
        if ($(v.element).length > 0 && typeof v.init === "function") {
        	SCCOMMON.ui[n].element = $(v.element);
            v.init();
        }
    })
};
