	...
	return {
		...
		displayLogin : function(targetPipeline) {
	    	...
	    	var el = document.createElement("iframe");
	    		...
	    		el.setAttribute('width', '652');
	    		el.setAttribute('height', '265');
...
jQuery(document).ready(function(){
	...
	$('#countryList').mouseenter(function(){
		...
		if( top == "18px" ) {
			that.stop().animate({'height': '183px','top':'-168px'}, 600, function(){			
				$('#countryList .selected').addClass('dwn');
			});
			...
			that.find('.hideable').css({'display':'list-item'}).click(function(){				
				...
				$(this).clone().prependTo("#countryList");				
				$('#countryList .selected').addClass('dwn');
				$(this).remove();
	...
	$('#countryList').mouseleave(function(){
		var that = jQuery(this);
		that.stop().animate({'height':'18px','top':'-3px'}, 600, function(){
			that.find('.hideable').hide();
			$('#countryList .selected').removeClass('dwn');
		});
